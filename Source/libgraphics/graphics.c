...
double ScaleXInches(int x) { return (double)x / GetXResolution(); }
...
void loadImage(const char *image, LibImage *mapbuf) {
  ...
  if (mapbuf->hbitmap != NULL) DeleteObject(mapbuf->hbitmap);
  mapbuf->hbitmap = CreateCompatibleBitmap(GetDC(graphicsWindow), mapbuf->width,
                                           mapbuf->height);
  SelectObject(hmapdc, mapbuf->hbitmap);

  ipicture->lpVtbl->Render(ipicture, hmapdc, 0, 0, mapbuf->width,
                           mapbuf->height, 0, height, width, -height, NULL);
