...
inline int DBExists(Model model) { return (model >= 0 && model <= N_MODEL); }
...
int Find(ListNode** target, unsigned int id, Model model) {
  if (model == BOOK) {
    ...
    if (cur == DBs[model].data->dummy_tail)
...
int Create(void* handle, Model model) {
  ...
  if (InsertList(DBs[model].data, DBs[model].data->dummy_tail, target) ==
      DBs[model].data->dummy_tail) {
...
int GetById(void* handle, unsigned int id, Model model) {
  ...
  if (ok == 0) {
    if (model == BOOK)
      return BookCopy((Book*)handle, (Book*)cur->value);
    else if (model == USER)
      return UserCopy((User*)handle, (User*)cur->value);
    else if (model == USER)
      return RecordCopy((BorrowRecord*)handle, (BorrowRecord*)cur->value);
  }
  return DB_NOT_EXISTS;
}
