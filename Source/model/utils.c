...
int BookCopy(Book* destination, Book* source) {
  ...
  t->uid = s->uid;
  err = SaveStrCpy(t->id, s->id);
  if (err != DB_SUCCESS) return err;
  err = SaveStrCpy(t->title, s->title);
  if (err != DB_SUCCESS) return err;
  int i;
  for (i = 0; i < 3; i++) {
    err = SaveStrCpy(t->authors[i], s->authors[i]);
    if (err != DB_SUCCESS) return err;
  }
  err = SaveStrCpy(t->category, s->category);
  if (err != DB_SUCCESS) return err;
  err = SaveStrCpy(t->press, s->press);
  if (err != DB_SUCCESS) return err;
  err = SaveStrCpy(t->publication_date, s->publication_date);
  if (err != DB_SUCCESS) return err;
  for (i = 0; i < 5; i++) {
    err = SaveStrCpy(t->keywords[i], s->keywords[i]);
    if (err != DB_SUCCESS) return err;
  }
...
int UserCopy(User* destination, User* source) {
  ...
  err = SaveStrCpy(t->id, s->id);
  if (err != DB_SUCCESS) return err;
  err = SaveStrCpy(t->salt, s->salt);
  if (err != DB_SUCCESS) return err;
  err = SaveStrCpy(t->name, s->name);
  if (err != DB_SUCCESS) return err;
  int i;
  for (i = 0; i < 8; i++) t->password[i] = s->password[i];
  t->gender = s->gender;
  err = SaveStrCpy(t->department, s->department);
  if (err != DB_SUCCESS) return err;
  t->whoami = s->whoami;
...
int RecordCopy(BorrowRecord* destination, BorrowRecord* source) {
  ...
  err = SaveStrCpy(t->book_id, s->book_id);
  if (err != DB_SUCCESS) return err;
  err = SaveStrCpy(t->user_id, s->user_id);
  if (err != DB_SUCCESS) return err;
  err = SaveStrCpy(t->borrowed_date, s->borrowed_date);
  if (err != DB_SUCCESS) return err;
  t->book_status = s->book_status;
  err = SaveStrCpy(t->returned_date, s->returned_date);
  if (err != DB_SUCCESS) return err;
  return DB_SUCCESS;
}
...
int BookFilter(Book* p_b, String queries) {
  ...
  char *property, *para;
  int insensitive = 0, equal = 1, flag = 1;
  char q[1000];
  strcpy(q, queries);
  property = strtok(q, "=");
  while (1) {
