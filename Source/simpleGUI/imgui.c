...
int button(int id, double x, double y, double w, double h, char *label) {
  ...
  if (gs_UIState.kbdItem == 0) gs_UIState.kbdItem = id;
...
static char shortcutkey(char *s) {
  ...
  if (n < M || strncmp(s + n - M, predStr, M - 1) != 0) return 0;
...
int menuList(int id, double x, double y, double w, double wlist, double h,
  ...
  if (menuItem(id, x, y, w, h, labels[0])) unfoldMenu = !unfoldMenu;
...
int textbox(int id, double x, double y, double w, double h, char textbuf[],
  ...
  if (gs_UIState.kbdItem == id) labelColor = gs_textbox_color.hotLabel;
